...
interface SingleEmailReportCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  studentId: string;
  studentName: string;
...
export default function SingleEmailReportCardModal({
  isOpen,
  onClose,
  studentId,
  studentName,
  term,
  onEmailSent
}: SingleEmailReportCardModalProps) {
  ...
  const fetchStudentData = useCallback(async () => {
    try {
      ...
      if (response.status === 'success') {
        ...
        // Set default subject for report card
        setCustomHeader(generateDefaultSubject(studentName, 'report_card', term));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 'success') {
        showNotification(`Report card email sent successfully to ${emailAddresses.length} recipient${emailAddresses.length > 1 ? 's' : ''}`, 'success');
        onEmailSent?.();
        onClose();
      } else {
      ...
      showNotification('Failed to send report card email', 'error');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex items-center gap-3 mb-6 p-4 bg-blue-50 rounded-lg">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              Email report card for {studentName} - {term}
            </p>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="mb-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                  ...
                  <p className="text-sm text-blue-700 italic">
                    &quot;Dear Parent/Guardian, Please find attached the report card for {studentName} for {term}. If you have any questions about your child&apos;s progress, please don&apos;t hesitate to contact us.&quot;
                  </p>
              ...
              <div className="flex items-start gap-3 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-yellow-700">
                    Please ensure the report card has been generated and the email addresses are correct.
                  </p>
                </div>
              </div>
