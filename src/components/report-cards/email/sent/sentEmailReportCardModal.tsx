...
interface SentEmailReportCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  studentId: string;
  studentName: string;
...
export default function SentEmailReportCardModal({
  isOpen,
  onClose,
  studentId,
  studentName,
  term
}: SentEmailReportCardModalProps) {
  ...
  const fetchEmailRecord = async () => {
    try {
      ...
      if (response.status === 'success') {
        // Find the report card email for this specific term
        const reportCardEmail = response.data.find(
          email => email.reportType.toLowerCase() === 'report_card' && email.term === term
        );

        if (reportCardEmail) {
          setEmailRecord(reportCardEmail);
        } else {
          showNotification('No email record found for this report card', 'error');
        }
      }
    } catch (error) {
      ...
      <div className="text-black">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <span className="text-sm text-gray-900">Report Card</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">
              We couldn&apos;t find an email record for this report card.
            </p>
          </div>
        )}
      </div>
