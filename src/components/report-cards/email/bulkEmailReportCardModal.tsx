...
interface BulkEmailReportCardModalProps {
  isOpen: boolean;
  onClose: () => void;
  availableReports: ReportCardRecord[];
  term: string;
  onEmailsSent?: () => void;
}

export default function BulkEmailReportCardModal({
  isOpen,
  onClose,
  availableReports,
  term,
  onEmailsSent
}: BulkEmailReportCardModalProps) {
  ...
  const reportsByGrade = availableReports.reduce((acc, report) => {
    ...
    return acc;
  }, {} as Record<string, ReportCardRecord[]>);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 'completed') {
        ...
        if (failureCount === 0) {
          showNotification(`Successfully sent ${successCount} report card emails`, 'success');
        } else if (successCount === 0) {
          showNotification(`Failed to send all ${failureCount} report card emails`, 'error');
        } else {
      ...
      showNotification('Failed to send bulk report card emails', 'error');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex items-center gap-3 mb-6 p-4 bg-blue-50 rounded-lg">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900">
              Send Bulk Report Card Emails
            </h3>
            <p className="text-sm text-gray-600">
              Select students to email report cards for {term}
            </p>
          </div>
        </div>
                ...
                <div className="max-h-64 overflow-y-auto border border-gray-300 rounded-md">
                  {availableGrades.length === 0 ? (
                    <div className="p-4 text-center text-gray-500">
                      No report cards available for bulk email
                    </div>
                  ) : (
                    availableGrades.map(grade => {
                      const gradeReports = reportsByGrade[grade] || [];
