...
import { createJKSKDomain } from '@/services/jkskService'
import Spinner from '@/components/Spinner'

interface JKSKDomainAddModalProps {
  isOpen: boolean
  onClose: () => void
...
export default function JKSKDomainAddModal({
  isOpen,
  onClose,
  documentType,
  school,
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const res = await createJKSKDomain({
        documentType,
        name: name.trim(),
      ...
      if (res.status === 'success') {
        showNotification('Domain created', 'success')
        setName('')
        onAdded()
        onClose()
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Add ${documentTypeLabel} Domain`} size="sm">
