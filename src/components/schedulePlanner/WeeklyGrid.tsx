...
// Minute-proportional weekly schedule grid. One component powers the admin
// workspace (interactive pinning), the public share page (read-only), and a
// compact mode for the teacher dashboard widget.
...
/** Assigns overlapping sessions of one day to side-by-side lanes. */
...
const WeeklyGrid: React.FC<WeeklyGridProps> = ({
  ...
  const gridHeight = compact ? 220 : Math.max(420, (span / 60) * 72)

  return (
    <div className="flex w-full select-none" style={{ height: gridHeight }}>
      {/* Hour ruler */}
      <div className={`${compact ? 'w-10' : 'w-14'} shrink-0 flex flex-col`}>
        <div className={`${compact ? 'h-6' : 'h-8'}`} />
      ...
      {days.map((day) => {
        ...
        const lanes = packLanes(daySessions)
        const dayBlocks = fixedBlocks.filter((b) => b.day === day)
        const fillable = fillableRangesByDay?.[day]

        return (
          <div key={day} className="flex-1 min-w-0 flex flex-col border-l border-gray-200 last:border-r">
            <div
              className={`${compact ? 'h-6 text-[10px]' : 'h-8 text-xs'} flex items-center justify-center font-semibold bg-cyan-50 text-cyan-900 border-b border-gray-200`}
            >
              {dayLabel(day, compact)}
            </div>
            <div className="relative flex-1 bg-white">
              ...
              {dayBlocks.map((b, i) => (
                ...
                >
                  <span className="text-[9px] font-medium text-gray-500 truncate px-1">{b.label}</span>
                </div>
              ))}
              {/* Sessions */}
              {daySessions.map((s) => {
                ...
                return (
                  <div
                    key={s.id}
                    className={`absolute rounded border border-black/10 overflow-hidden px-1 py-0.5 group ${
                  ...
                  >
                    {s.pinned && (
                      <MapPinIcon className="absolute top-0.5 right-0.5 h-3 w-3 text-cyan-700" />
                    )}
                    <div className={`font-semibold truncate ${compact ? 'text-[8px]' : 'text-[10px]'}`}>
                      {s.title}
                    </div>
                    {!compact && (
                    ...
                    {!compact && s.subtitle && (
                      <div className="text-[8px] text-gray-600 truncate">{s.subtitle}</div>
                    )}
                    {!compact && s.roomName && (
                      <div className="text-[8px] text-gray-500 truncate">{s.roomName}</div>
                    )}
                  </div>
                )
              })}
